...
export async function saveAthleteProfile(athlete:PassportAthlete){try{await updateCurrentAthlete({displayName:athlete.name,countryCode:athlete.countryCode,manualCity:athlete.citySource==="manual"?athlete.city:null,timezone:Intl.DateTimeFormat().resolvedOptions().timeZone,trainingContext:trainingMap[athlete.trainingContext],preferredMachineKey:athlete.preferredMachineId,preferredMachineLabel:athlete.preferredMachine,bio:athlete.biography,passportVisibility:visibilityMap[athlete.visibility]});revalidatePath("/passport");revalidatePath("/dashboard");return {persisted:true,message:"Athlete Passport saved."};}catch(error){console.error("Passport update failed",error instanceof Error?error.message:"unknown");return {persisted:false,message:"Your Passport could not be saved. Please retry."};}}
