...
export function RankingsExplorer({initialQuery}:{initialQuery:Record<string,string|string[]|undefined>}) {
  ...
  return <div><header className="rowing-lanes wake-lines relative overflow-hidden rounded-[2rem] bg-[#0d2b24] p-6 text-white shadow-[0_30px_80px_rgba(13,43,36,.18)] sm:p-10"><p className="text-xs font-black uppercase tracking-[.18em] text-[#ff9a73]">Comparable performance</p><h1 className="display-type mt-4 max-w-4xl text-5xl font-black leading-none sm:text-7xl">See where every stroke stands.</h1><p className="mt-5 max-w-3xl text-sm leading-7 text-white/70">Every table states its geography, division, machine class, verification provenance, and governing definition. Provider-neutral history does not imply cross-machine equivalence.</p></header>
    ...
    <section className="mt-5 overflow-hidden rounded-3xl border border-[#dfe5e1] bg-white" aria-label={`${filters.scope} ranking results`}>{rows.length?rows.map((row,index)=>{const provider=getMachineProvider(row.providerId);const model=getMachineModel(row.modelId);const comparison=comparabilityLabel(row.providerId,row.modelId,row.machineClassId,filters);return <Link key={row.id} href={`/results/${row.id}`} className={`grid grid-cols-[42px_1fr_auto] items-center gap-3 border-b border-[#e7ebe8] p-4 last:border-0 hover:bg-[#fbfcfb] focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-[#16725e] sm:px-6 ${row.current?"bg-[#fff8f4]":""}`} aria-label={`View detailed result for ${row.athleteName}`}><div className="text-center font-black text-[#718078]">{index===0?<Crown className="mx-auto text-[#d79a20]" size={20} aria-label="First place"/>:index+1}</div><div className="flex min-w-0 items-center gap-3"><span className="grid size-10 shrink-0 place-items-center rounded-full bg-[#dceae5] text-xs font-black">{row.initials}</span><div className="min-w-0"><h2 className="truncate font-black">{row.athleteName}{row.current&&<span className="ml-2 text-xs text-[#d94d1c]">You</span>}</h2><p className="mt-1 flex items-center gap-1 truncate text-xs text-[#718078]"><MapPin size={12} aria-hidden="true"/>{row.country}{row.club?` · ${row.club}`:" · Independent"}</p><p className="mt-1 text-[10px] font-bold text-[#475b54]">{provider?.displayName??"Unknown"} {model?.model??"Unknown model"} · {labelFor(MACHINE_CLASSES,row.machineClassId)}</p><p className="mt-1 text-[10px] font-bold text-[#718078]">{comparison} · {labelFor(OFFICIAL_STATUSES,row.officialStatusId)}</p></div></div><div className="text-right"><p className="font-black tabular-nums">{formatRankingResult(row)}</p><p className="mt-1 flex items-center justify-end gap-1 text-[10px] font-bold text-[#16725e]"><ShieldCheck size={11}/>{labelFor(VERIFICATION_OPTIONS,row.verificationId)}</p></div></Link>}):<div className="p-10 text-center"><p className="font-black">No comparable results match this definition.</p><p className="mt-2 text-sm text-[#718078]">Try another division, format, geography, brand, model, machine class, or verification tier.</p></div>}</section>
