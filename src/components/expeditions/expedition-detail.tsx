    ...
    <div>
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-[1.25fr_.75fr]">
        ...
        <aside className="space-y-5">
          <section className="rounded-3xl border border-[#dfe5e1] bg-white p-5 sm:p-6"><p className="text-xs font-black uppercase tracking-[.16em] text-[#d94d1c]">Recent contributions</p><h2 className="mt-2 text-xl font-black">Workouts moving the route</h2>{recentContributions.length > 0 ? <div className="mt-5 space-y-3">{recentContributions.map((item) => <Link key={item.id} href={`/workouts/${item.workoutId}`} className="block rounded-2xl bg-[#f5f7f4] p-4 hover:bg-[#eef3f0] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#16725e]" aria-label={`View workout details for ${item.title}`}><div className="flex justify-between gap-3"><div><p className="text-sm font-black">{item.title}</p><p className="mt-1 text-xs text-[#718078]">{item.occurredOn} · {item.source}</p></div><p className="text-sm font-black tabular-nums text-[#16725e]">+{km(item.distanceMeters)}</p></div><p className="mt-2 text-[10px] text-[#8a9691]">Contribution references the authoritative workout; Expedition progress remains separate.</p></Link>)}</div> : <p className="mt-4 text-sm leading-6 text-[#718078]">Start this Expedition to send future eligible workout distance here. Earlier workouts are not applied retroactively.</p>}</section>
