    ...
    <div className="space-y-5 sm:space-y-6">
      ...
      <div className="grid gap-5 xl:grid-cols-[1.08fr_.92fr]">
        <PassportSection id="expeditions" eyebrow="Journey record" title="Expeditions" description="Real eligible distance, place-based stories, and completion artifacts carried in your Passport." action={<Route size={22} className="text-[#d94d1c]" aria-hidden="true" />}>
          <article className="mt-6 rounded-2xl bg-[#0d2b24] p-5 text-white sm:p-6"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-[10px] font-black uppercase tracking-[0.14em] text-[#8eb4a8]">Active · Ambitious</p><h3 className="mt-2 text-2xl font-black">{expedition.name}</h3><p className="mt-1 text-sm text-[#a9c1ba]">{expedition.subtitle}</p></div><p className="text-3xl font-black text-[#ff9a73]">{expeditionProgress}%</p></div><div className="mt-6 h-2 overflow-hidden rounded-full bg-white/15" role="progressbar" aria-label={`${expeditionProgress} percent of The Danube completed`} aria-valuemin={0} aria-valuemax={100} aria-valuenow={expeditionProgress}><div className="h-full rounded-full bg-[#ff6b35]" style={{ width: `${expeditionProgress}%` }} /></div><div className="mt-5 grid grid-cols-2 gap-4 text-sm"><div><p className="text-[#8eb4a8]">Current position</p><p className="mt-1 font-black">{expedition.currentPosition}</p></div><div><p className="text-[#8eb4a8]">Next landmark</p><p className="mt-1 font-black">{expedition.nextLandmark} · {expedition.distanceToNextKm} km</p></div><div><p className="text-[#8eb4a8]">Distance completed</p><p className="mt-1 font-black">{expedition.completedKm} / {expedition.totalKm.toLocaleString()} km</p></div></div><Link href="/challenges" className="mt-6 inline-flex min-h-11 items-center gap-2 rounded-full bg-white px-4 text-sm font-black text-[#0d2b24]">Open Expedition <ArrowRight size={16} aria-hidden="true" /></Link></article>
        ...
        <PassportSection id="timeline" eyebrow="Rowing journey" title="Athlete timeline" description="Milestones from training, Expeditions, community, and competition—not a social feed.">
          <ol className="relative mt-6 space-y-0 border-l border-[#cdd9d4] pl-6">
            {passportData.timeline.map((entry) => <li key={entry.id} className="relative pb-5 last:pb-0"><span className="absolute -left-[1.78rem] top-1 size-3 rounded-full border-2 border-white bg-[#ff6b35] ring-1 ring-[#d5dfdb]" /><time className="text-[10px] font-black uppercase tracking-[0.1em] text-[#718078]">{entry.date}</time><h3 className="mt-1 text-sm font-black">{entry.title}</h3><p className="mt-1 text-xs leading-5 text-[#718078]">{entry.detail}</p></li>)}
          </ol>
        </PassportSection>
      </div>
      ...
      <PassportSection id="competitions" eyebrow="Competition record" title="Results with credible provenance" description="Each performance remains tied to its event rules, category, machine class, and verification tier.">
        <div className="mt-6 grid gap-3 lg:grid-cols-3">{passportData.competitions.map((record) => <article key={record.id} className="rounded-2xl border border-[#e2e7e4] p-4"><div className="flex items-start justify-between gap-3"><Medal size={19} className="text-[#d39820]" aria-hidden="true" /><time className="text-xs text-[#718078]">{record.date}</time></div><h3 className="mt-4 font-black">{record.event}</h3><p className="mt-1 text-xs text-[#718078]">{record.format}</p><p className="mt-4 text-3xl font-black tabular-nums">{record.result}</p><p className="mt-1 text-sm font-bold text-[#16725e]">{record.placing}</p><dl className="mt-4 space-y-2 border-t border-[#e7ebe8] pt-4 text-xs"><div><dt className="text-[#718078]">Category</dt><dd className="mt-0.5 font-bold">{record.category}</dd></div><div><dt className="text-[#718078]">Machine class</dt><dd className="mt-0.5 font-bold">{record.machineClass}</dd></div></dl><div className="mt-4"><VerificationBadge label={record.verification} /></div></article>)}</div>
      </PassportSection>
